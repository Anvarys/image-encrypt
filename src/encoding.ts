...
export async function encode_file(canvas: HTMLCanvasElement, file: File, color_bits_used: number, pixel_offset: number = 0) {
  ...
  if (pixel_count < file.size * (8.0 / color_bits_used) / 3.0) {
    toast.error(`Not enough pixels on the image to encode the file (${((pixel_count/(file.size * (8.0 / color_bits_used) / 3.0))*100).toFixed(2)}%)`);
    return null;
  }
